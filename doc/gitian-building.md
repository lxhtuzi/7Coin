...
*Setup instructions for a Gitian build of 7Coin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the 7Coin
...
- [Building 7Coin Core](#building-7Coin-core)
...
Clone the git repositories for 7Coin and Gitian.
...
```bash
sudo cp /home/debian/7Coin/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the 7Coin repository under 'Fetch and create inputs' to install sources which require
...
To build 7Coin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the 7Coin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/7Coin/.git/
...
For example:
```bash
URL=https://github.com/laanwj/7Coin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit 7Coin=${COMMIT} --url 7Coin=${URL} ../7Coin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit 7Coin=${COMMIT} --url 7Coin=${URL} ../7Coin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit 7Coin=${COMMIT} --url 7Coin=${URL} ../7Coin/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-7Coin:
./bin/gbuild --commit 7Coin=${COMMIT},cpp-eth-7Coin=${ETHCOMMIT} --url 7Coin=${URL},cpp-eth-7Coin=${ETHURL} ../7Coin/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the 7Coin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../7Coin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/7Coin
SIGPATH=/some/root/path/7Coin-detached-sigs

./bin/gbuild --url 7Coin=${BTCPATH},signature=${SIGPATH} ../7Coin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/7Coin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/7Coin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/7Coin-osx-build.assert
```
...
[7Coinproject/gitian.sigs](https://github.com/7Coinproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@7Coin.org) and he will commit them.
